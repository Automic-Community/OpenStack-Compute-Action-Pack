...
/**
 * This class is used to Creates one or more servers with an optional reservation ID. It publish one or more created
 * server id.
 * 
...
public class CreateServerAction extends AbstractHttpAction {
    ...
    protected void validate() throws AutomicException {
        ...
        if (!Validator.checkFileExists(parameterFilePath)) {
            String errMsg = String.format(ExceptionConstants.INVALID_FILE, parameterFilePath);
            LOGGER.error(errMsg);
    ...
    protected void executeSpecific() throws AutomicException {
        ...
        response = webResource.entity(validateJsonObject(parameterFilePath), MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON).header(Constants.X_AUTH_TOKEN, tokenId).post(ClientResponse.class);
    ...
    private void prepareOutput(ClientResponse response) throws AutomicException {
        ...
        if (jsonObj != null && jsonObj.has(SERVER_KEY)) {
            ...
            params.put(RESERVATION_ID, Arrays.asList(jsonObj.getString(RESERVATION_ID)));

            ListServerService rps = ListServerService.getListServerService(client);
            jsonObj = rps.executeListServerService(baseUrl, tenantId, tokenId, params);

            if (jsonObj != null && jsonObj.has(SERVERS_KEY)) {
                publishSrversId(jsonObj.getJSONArray(SERVERS_KEY));
            } else {
                LOGGER.error(" Unable to find Json key[servers] in json object :" + jsonObj);
                throw new AutomicException(ExceptionConstants.INVALID_RESPONSE);
            }

        } else {
            LOGGER.error(" Unable to find Json keys[server, reservation_id] in json object :" + jsonObj);
            throw new AutomicException(ExceptionConstants.INVALID_RESPONSE);
        }
    ...
    /**
     * This method validate and modify {@code JSONObject} instance integrity Should be Valid json If min_count or
     * max_count > 1, then put reservation_id max_count should be greater than or equal to min_count
     * 
     * @param parameterFilePath
     * @return {@code String} representation of {@code JSONObject} instance
     * @throws AutomicException
     */
    private String validateJsonObject(String parameterFilePath) throws AutomicException {
        JSONObject jsonObj = CommonUtil.getJSONObjectByFilePath(parameterFilePath);
        if (jsonObj != null) {
            JSONObject server = jsonObj.getJSONObject(SERVER_KEY);
            if (server != null) {
                // If min_count not specify, put min_count as 1
                int minCount = server.has(MIN_COUNT_KEY) ? server.getInt(MIN_COUNT_KEY) : 1;
                // If max_count not specify, put max_count as min_count
                int maxCount = server.has(MAX_COUNT_KEY) ? server.getInt(MAX_COUNT_KEY) : minCount;

                if (minCount > maxCount) {
                    LOGGER.error(ExceptionConstants.MAXCOUNT_LESS_MINCOUNT);
                    throw new AutomicException(ExceptionConstants.MAXCOUNT_LESS_MINCOUNT);
                }
                if (maxCount > 30) {
